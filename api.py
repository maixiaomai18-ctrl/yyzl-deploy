...
def search(username: str, q: str, view_as: str = ""):
    ...
    # 把关键词按空格拆成多个词,每个词都要命中(AND);解决"整段超长文字搜不到"的问题
    words = [w for w in keyword.split() if w]
    if not words:
        words = [keyword]
    conds = " AND ".join(["m.content ILIKE %s"] * len(words))
    params = [target] + [f"%{w}%" for w in words]

    conn = db(); cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    cur.execute(f"""
        SELECT a.id AS account_id, ad.id AS adv_id, a.dept, a.account_name,
