...
def patch_templates(content):
    ...
    # 老版完整代码 (用 raw 精确匹配避免歧义)
    old = '''def session_revoked_alert(phone, account_name):
    return (
        f"⚠️ 【TG 会话被吊销】\\n\\n"
        f"外事号: {account_name}\\n"
        f"手机: {phone}\\n\\n"
...
def session_restored_alert(phone, account_name):
    ...
    new = '''def session_revoked_alert(phone, account_name, vps_label="", operator_name="", web_url=""):
...
def session_restored_alert(phone, account_name, vps_label="", operator_name="", web_url=""):
    ...
    if old not in content:
        # 已经升级过?
        if 'def session_revoked_alert(phone, account_name, vps_label=' in content:
            print(f"{C.Y}  templates.py: 已升级过,跳过{C.END}")
            return content
        raise RuntimeError("templates.py 找不到原版 session_revoked_alert/session_restored_alert (可能已被改过)")
